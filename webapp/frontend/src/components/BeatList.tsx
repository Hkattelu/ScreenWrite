  ...
  return (
    <div className="w-full relative">
      ...
      <div className="flex items-center gap-10 py-6 border-b border-gray-100 mb-8 sticky top-16 bg-white/90 backdrop-blur-md z-20">
        ...
        <div className="hidden md:flex items-center gap-3 px-4 py-2 bg-gray-50 rounded-lg border border-gray-100">
          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Shortcuts</span>
          <div className="flex gap-1">
            <kbd className="px-1.5 py-0.5 rounded border border-gray-200 bg-white text-[10px] font-bold text-gray-500 shadow-sm">↑</kbd>
            <kbd className="px-1.5 py-0.5 rounded border border-gray-200 bg-white text-[10px] font-bold text-gray-500 shadow-sm">↓</kbd>
          </div>
        </div>
      ...
      <div className="relative pl-12 md:pl-24 pt-4">
        {/* Timeline Head (Start) */}
        <div className="absolute left-6 md:left-12 -top-2 flex flex-col items-center -translate-x-1/2">
          <div className="w-1.5 h-1.5 rounded-full bg-gray-200" />
          <span className="text-[8px] font-black text-gray-300 uppercase tracking-widest mt-1">START</span>
        </div>

        {/* Vertical Timeline Track */}
        <div className="absolute left-6 md:left-12 top-2 bottom-6 w-0.5 bg-gray-300 -translate-x-1/2 z-0" />
        
        {/* Progress Fill */}
        <motion.div 
          className="absolute left-6 md:left-12 top-2 w-0.5 bg-blue-500 z-10 origin-top -translate-x-1/2"
          initial={{ scaleY: 0 }}
        ...
        {/* Timeline Tail (End) */}
        <div className="absolute left-6 md:left-12 -bottom-2 flex flex-col items-center -translate-x-1/2 z-0">
          <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">END</span>
          ...
          {beats.map((beat, index) => {
            ...
            return (
              <div key={beat.id} className="relative">
                ...
                <div className="absolute -left-12 md:-left-24 top-6 flex flex-col items-center w-12 md:w-24 z-20">
                  <div className="relative flex items-center justify-center h-8 w-full">
                    {/* Timestamp Label */}
                    <div className="absolute -left-2 md:left-2 top-1/2 -translate-y-1/2 -translate-x-full pr-2 hidden md:block">
                    ...
                    <div className="relative flex items-center justify-center">
                      <motion.div 
                        ...
                        className={`
                          w-3 h-3 rounded-full border-2 transition-colors duration-300 z-30
                          ${isReviewed ? 'shadow-[0_0_15px_rgba(59,130,246,0.5)]' : ''}
                        `}
                      />
                      ...
                      {!isReviewed && (
                        <motion.div 
                          ...
                          className="absolute inset-0 w-3 h-3 rounded-full opacity-20"
                        />
                      )}
                    </div>
                    ...
                    <div className={`
                      absolute left-[50%] right-0 h-px transition-all duration-500 
                      ${isReviewed ? 'bg-blue-100 scale-x-100' : 'bg-gray-100 scale-x-75 opacity-50'}
                      origin-left
                    `} />
                  </div>
                </div>
                ...
                <motion.div 
                  ...
                  className={`
                    ...
                    ${isEditing ? 'bg-white ring-1 ring-gray-200 shadow-xl p-6 z-10' : 'bg-white p-5 border border-gray-100 hover:border-gray-300 focus-within:border-blue-300 focus-within:ring-4 focus-within:ring-blue-500/5'}
                    ...
                    {isEditing ? (
                      ...
                      <div className="flex items-stretch gap-6">
                        <button 
                          ...
                          className={`
                            w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 border flex-shrink-0 mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500
                            ${isReviewed 
                        ...
                        {editable && (
                          <div className="flex flex-col gap-2 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-all duration-200 border-l border-gray-100 pl-4 py-1">
                            <button
                              onClick={() => handleEdit(beat)}
                              aria-label="Edit beat"
                              className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-blue-600 hover:bg-blue-50 focus:bg-blue-50 focus:text-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
                              title="Edit Segment"
                            >
                              <Edit3 size={14} />
                            </button>
                            ...
                            {viewMode !== 'none' && (
                              <button
                                ...
                                className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-red-500 hover:bg-red-50 focus:bg-red-50 focus:text-red-500 focus:outline-none focus:ring-2 focus:ring-red-500 transition-all"
